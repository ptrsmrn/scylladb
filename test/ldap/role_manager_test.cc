...
SEASTAR_TEST_CASE(ldap_filter_injection_with_wildcard_user) {
    return do_with_cql_env_thread([](cql_test_env& env) {
        ...
        // Escaping keeps '*' literal, so it should not match any LDAP memberUid entries besides the local role itself.
...
SEASTAR_TEST_CASE(ldap_filter_injection_with_parenthesis_payload) {
    return do_with_cql_env_thread([](cql_test_env& env) {
        ...
        // With escaping, the payload becomes (memberUid=\29\28uid=\2a) - a valid filter
...
SEASTAR_TEST_CASE(ldap_filter_injection_with_percent_encoded_wildcard) {
    return do_with_cql_env_thread([](cql_test_env& env) {
        ...
        // Without URL percent-encoding of '%', ldap_url_parse would
...
SEASTAR_TEST_CASE(ldap_filter_injection_with_question_mark_user) {
    return do_with_cql_env_thread([](cql_test_env& env) {
        ...
        // '?' is the LDAP URL component separator.  Without URL
...
SEASTAR_TEST_CASE(ldap_rejects_user_outside_filter_component) {
    // {USER} placed outside the filter component (in the base DN here) is a
