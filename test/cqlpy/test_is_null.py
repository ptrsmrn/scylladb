...
def test_is_null_without_filtering_error(cql, table1):
    ...
    # IS NULL on a pk column without ALLOW FILTERING should fail.
    # This is a special case, since pk columns can never be null,
    # but since ALLOW FILTERING is required in general case,
    # we decided to not make the implementation more complex here.
    with pytest.raises(InvalidRequest, match='ALLOW FILTERING'):
...
def test_is_not_null_without_filtering_error(cql, table1):
    ...
    # IS NOT NULL on a pk column without ALLOW FILTERING should fail
    with pytest.raises(InvalidRequest, match='ALLOW FILTERING'):
