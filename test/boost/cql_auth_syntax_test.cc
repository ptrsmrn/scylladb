...
test_valid(std::string_view cql_query) {
    return test_valid(cql_query, &cql3_parser::CqlParser::query_stmt);
}

BOOST_AUTO_TEST_CASE(user_name) {
    const auto test = make_tester(&cql3_parser::CqlParser::username_ident);
    ...
    BOOST_REQUIRE_THROW(
            (cql3::util::do_with_parser("\"Ring-bearer\"", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::username_ident))),
...
BOOST_AUTO_TEST_CASE(permission_or_all) {
    const auto test = make_tester(&cql3_parser::CqlParser::permissionOrAll_perms);
...
BOOST_AUTO_TEST_CASE(resource_) {
    const auto test = make_tester(&cql3_parser::CqlParser::resource_res);
...
BOOST_AUTO_TEST_CASE(user_or_role_name) {
    const auto test = make_tester(&cql3_parser::CqlParser::userOrRoleName_name);
...
BOOST_AUTO_TEST_CASE(test_role_options) {
    // roleOptions is a void rule that modifies opts; wrap it to fit do_with_parser
    auto test = [](std::string_view fragment) {
        cql3::role_options opts;
        BOOST_REQUIRE_NO_THROW(
            cql3::util::do_with_parser(fragment, cql3::dialect{}, [&opts](cql3_parser::CqlParser& parser) {
                parser.roleOptions(opts);
                return 0;
            }));
    };

    test("PASSWORD = 'shire'");
    test("OPTIONS = { 'foo': 10, 'bar': 20 }");
    test("LOGIN = false");
    test("SUPERUSER = true");
    test("PASSWORD = 'shire' AND OPTIONS = { 'foo': 10, 'bar': 20 }");
    test("PASSWORD = 'shire' AND LOGIN = true AND SUPERUSER = false");
}
...
BOOST_AUTO_TEST_CASE(create_role_with_hashed) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH HASHED = 'something';", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query_stmt)),
        exceptions::syntax_exception);
}

BOOST_AUTO_TEST_CASE(create_role_with_hashed_underscore_password) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH HASHED_PASSWORD = 'something';", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query_stmt)),
        exceptions::syntax_exception);
}

BOOST_AUTO_TEST_CASE(create_role_with_hash) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH HASH = 'something';", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query_stmt)),
        exceptions::syntax_exception);
}

BOOST_AUTO_TEST_CASE(create_role_with_hashed_password_double_quotation_marks) {
    BOOST_REQUIRE_THROW(
        cql3::util::do_with_parser("CREATE ROLE jane WITH HASHED PASSWORD = \"something\";", cql3::dialect{}, std::mem_fn(&cql3_parser::CqlParser::query_stmt)),
        exceptions::syntax_exception);
}
